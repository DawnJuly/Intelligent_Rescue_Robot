...
def ball_in_roi(ls_balls, roi):
    ...
    return Our_balls,Yellow_balls,Black_balls
...
def find_ball(ls_ourballs, ls_safes, uart):
    # 用FB_ROI过滤上面，只关注下面的
    Ours, Yellows, Blacks = ball_in_roi(ls_ourballs, FB_ROI)
...
# --- A == 1：检查小球颜色 ---
def check_ball(ls_ourballs, catch, uart):
    # 只关注矩形框CB_ROI内的小球
    Our_balls,Yellow_balls,Black_balls=ball_in_roi(ls_ourballs, CB_ROI)
...
def run(our):
    ...
    while True:
        with ScopedTiming("total", 2):
            ...
            if A == 1:
                check_ball(ls_ourballs, catch, uart)
            # 寻找安全区
            if A == 2:
