...
confidence_threshold = 0.8  # 置信度阈值 模型输出的检测框置信度低于该值，会被直接过滤掉
nms_threshold = 0.8  # 非极大值抑制阈值 用于去除重叠的重复检测框：两个框重叠度（IOU）高于该值时，只保留置信度更高的那个
...
# 测量安全区距离
def safe_length(x1, y1, w, h, image_w=800, image_h=480, h_fov=65, v_fov=40):
    yaw, _= cal(x1, y1, w, h, image_w, image_h, h_fov)
...
# 判断小球在兴趣区域roi
CB_ROI = (200, 80, 400, 400) # (x1, y1, w, h)
...
# --- A == 0：找球模式 ---
def find_ball(ls_ourballs, ls_safes, uart):
    ls_ourballs = ball_in_roi(ls_ourballs, FB_ROI)
    # 过滤掉在安全区内的小球
    Our_balls = []
    Yellow_balls = []
    Black_balls = []
    for b in ls_ourballs:
        if not ball_in_safe(b, ls_safes):
            if b[-1] == 2:
                Yellow_balls.append(b)
            elif b[-1] == 3:
                Black_balls.append(b)
            else:
                Our_balls.append(b)

    if Yellow_balls or Black_balls:
        max_ls = find_max(Yellow_balls+Black_balls)
...
# --- A == 1：检查小球颜色 ---
def check_ball(ls_ourballs, ls_balls, catch, uart):
    # 只看 ROI 矩形框内的小球
    ls_balls = ball_in_roi(ls_balls, CB_ROI)
    totalnum = len(ls_balls)
    ls_ourballs = ball_in_roi(ls_ourballs, CB_ROI)
    ournum = len(ls_ourballs)

    # roi内的总球数不能大于1
    if totalnum == 1 and ournum == 1:
        catch_ball = ls_ourballs[0]
...
def run(our):
    ...
    while True:
        with ScopedTiming("total", 2):
            ...
            # 找球模式
            if A == 0:
                find_ball(ls_ourballs, ls_safes, uart)
            # 已经抓到小球，检查小球颜色
            if A == 1:
                check_ball(ls_ourballs, ls_balls, catch, uart)
            # 寻找安全区
            if A == 2:
