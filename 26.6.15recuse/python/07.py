    ...
    while True:
        with ScopedTiming("total", 2):
            ...
            # 已经抓到小球，检查小球颜色
                ...
                # 小球数量不是1个，或者小球过远，或者抓取的小球不在目标内，发送N，松开夹爪
                if num != 1 or length >= 35 or ls_balls[0][-1] not in catch:
                    time.sleep(0.3)
                    uart.write("[N]")
                    print("小球颜色错误")
                else:
            ...
            # 寻找安全区
                ...
                if S and send_count <= 0:
                    continue
                # 将安全区发送出去
                max_ls = find_max(ls_safe)
                if max_ls is not None:
                    id = max_ls[4]
                    yaw, length = cal(max_ls[0], max_ls[1], max_ls[2], max_ls[3])
                    send(uart, yaw, length, id)
                    print(f"{dic[id]}安全区，{send_count}次数")
                    # 如果处于倒数模式，每成功发送一次减1
                    if S:
                        send_count -= 1
            else:
