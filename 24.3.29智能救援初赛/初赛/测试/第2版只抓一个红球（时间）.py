import sensor, time  # 引入感光元件模块、时间模块
...
def find_safe(img, K):
    safes = []
    for red_blob in img.find_blobs([red_threshold], area_threshold=200, pixels_threshold=200, merge=True):
        red_rect = red_blob.rect()  # 绘制红色矩形框
        for purple_blob in img.find_blobs([purple_threshold], area_threshold=80, pixels_threshold=80, merge=True):
