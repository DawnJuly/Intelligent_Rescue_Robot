...
def find_safe(img, K):
    safes = []
    for red_blob in img.find_blobs([red_threshold], area_threshold=80, pixels_threshold=80, merge=True):
        red_rect = red_blob.rect()  # 绘制红色矩形框
        for purple_blob in img.find_blobs([purple_threshold], area_threshold=200, pixels_threshold=200, merge=True):
            ...
            if (purple_rect[0] <= red_rect[0] and  # 判断紫色矩形框的左上角 x 坐标是否小于等于红色矩形框的左上角 x 坐标
                ...
                # 计算安全区距离
                Lm = (red_blob[2] + red_blob[3]) / 2
                length = K / Lm

                safes.append((x, length))
    return safes
...
while True:
    ...
    # 寻找安全区
    red_safe = find_safe(img, K)
    for x, length in red_safe:
        # 判断是否进入安全区
        if length <= 5:
            uart.write("!")
            print("成功进入安全区")
