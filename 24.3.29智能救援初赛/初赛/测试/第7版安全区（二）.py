...
color_thresholds = {
    "Red": (20, 80, 10, 80, 10, 80),
    "Blue": (3, 50, -60, 60, -60, -5),
    "Yellow": (40, 100, -80, 10, 30, 100),
    "Black": (0, 25, -5, 5, -1, 10),
}
...
purple_threshold = (10, 60, -60, 60, -80, -10)
...
def safe(img, color):
    ...
    for blob in img.find_blobs([threshold], area_threshold=3000, pixels_threshold=3000, merge=True):
        rect = blob.rect()  # 绘制矩形框
        for purple_blob in img.find_blobs([purple_threshold], area_threshold=500, pixels_threshold=500, merge=True):
