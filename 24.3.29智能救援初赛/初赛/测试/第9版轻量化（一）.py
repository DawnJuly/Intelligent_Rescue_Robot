...
# 处理小球
def process_balls(uart, img, K, right_balls, wrong_balls):
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if time.ticks_diff(current_time, last_time) >= interval:
            if round_num == 1:
                if process_balls(uart, img, K,["Red"], ["Blue", "Yellow", "Black"]):
                    break
            elif round_num == 2:
                if process_balls(uart, img, K,["Red", "Yellow", "Black"], ["Blue"]):
                    break
        else:
            time.sleep(0.1)
