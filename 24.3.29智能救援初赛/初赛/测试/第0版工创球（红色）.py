import sensor, time  # 引入感光元件模块、拍照模块和时间模块
from pyb import UART

# 设置摄像头
sensor.reset()  # 初始化感光元件
sensor.set_pixformat(sensor.RGB565)  # 设置为彩色
sensor.set_framesize(sensor.QVGA)  # 设置图像的大小，VGA是640*480分辨率，QVGA是320*240分辨率，QQVGA是160*120分辨率
sensor.skip_frames(time=2000)  # 跳过一些帧，等待感光元件变稳定
clock = time.clock()

sensor.set_auto_gain(False)  # 将自动增益功能设置为关闭状态
sensor.set_auto_whitebal(False)  # 将自动白平衡功能设置为关闭状态

# 定义颜色阈值字典
# Lab颜色空间中，L亮度；a的正数代表红色，负端代表绿色；b的正数代表黄色，负端代表蓝色
color_thresholds = {
    "Red": (10, 40, 20, 80, -80, 80),
    "Blue": (0, 35, -40, 10, -40, -5),
    "Yellow": (40, 80, -40, 40, 15, 80),
    "Black": (0, 5, -10, 10, -10, 10),
    "Purple":(20, 80, 10, 127, -128, -20)
}

# 初始化串口
uart = UART(3, 115200)  # 使用串口编号为3，波特率为115200

# 定义一个函数用于通过串口发送横坐标x和距离length
def send_coordinates(uart, x, length, color):
    try:
        length = round(length)  # 四舍五入保留整数
        uart.write("@{},{}#".format(x, length))
        print("{}发送成功，距离{}".format(color,length))
    except:
        print("发送失败")

# 定义一个函数用于接收信息
def receive(uart, r):
    if uart.any():
        data = uart.read(1)  # 读取一个字节的数据
        if data == "%":
            r = r+1
    return r

# 定义一个函数用于寻找某颜色小球
def find_ball(img, threshold, color_name, K):
    balls = []
    """
    使用find_blobs方法在图像中查找符合阈值条件的色块
    area_threshold 面积阈值，如果色块被框起来的面积小于这个值，会被过滤掉
    pixels_threshold 像素个数阈值，如果色块像素数量小于这个值，会被过滤掉
    merge 合并，如果设置为True，那么合并所有重叠的blob为一个
    """
    for blob in img.find_blobs([threshold], area_threshold=80, pixels_threshold=80, merge=True):
        # 检查色块的圆度是否大于 0.65，以此判断是否为小球
        if blob.roundness() > 0.65:
            # 获取小球的中心点坐标
            x = blob.cx()
            y = blob.cy()
            # 在图像上绘制矩形框，框住检测到的小球
            img.draw_rectangle(blob.rect())
            # 在小球的中心点绘制十字标记
            img.draw_cross(x, y)

            # 给矩形框备注
            m = blob.rect()[0]
            n = blob.rect()[1] - 10  # 往上偏移10个像素
            img.draw_string(m, n, f"{color_name}")

            # 计算小球距离
            Lm = (blob[2]+blob[3])/2
            length = K/Lm

            balls.append((x, length))
    return balls

# 定义一个函数用于寻找安全区
def find_safe(img, uart):
    # 查找蓝色区域
    red_blobs = img.find_blobs([red_threshold], x_stride=5, y_stride=5, area_threshold=200, pixels_threshold=200, merge=True)
    # 查找紫色边框
    purple_blobs = img.find_blobs([purple_threshold], x_stride=5, y_stride=5, area_threshold=200, pixels_threshold=200, merge=True)

    for red_blob in red_blobs:
        red_rect = red_blob.rect()  # 绘制红色矩形框
        for purple_blob in purple_blobs:
            purple_rect = purple_blob.rect()  # 绘制紫色矩形框
            # 矩形框通常用一个四元组 (x, y, w, h) 来表示，其中 x 和 y 是矩形框左上角的坐标，w 是矩形框的宽度，h 是矩形框的高度
            # 判断紫色边框是否围绕红色区域
            if (purple_rect[0] <= red_rect[0] and  # 判断紫色矩形框的左上角 x 坐标是否小于等于红色矩形框的左上角 x 坐标
                purple_rect[1] <= red_rect[1] and  # 判断紫色矩形框的左上角 y 坐标是否小于等于红色矩形框的左上角 y 坐标
                # 判断紫色矩形框的宽度是否大于等于红色矩形框的宽度
                purple_rect[2] >= red_rect[2] and
                # 判断紫色矩形框的高度是否大于等于红色矩形框的高度
                purple_rect[3] >= red_rect[3]):
                # 获取安全区的中心点坐标
                x = red_blob.cx()
                y = red_blob.cy()
                # 调用发送坐标信息的函数
                # send_coordinates(uart, x, y)
                # 在图像上绘制矩形框，框住检测到的安全区
                img.draw_rectangle(red_blob.rect())
                # 在安全区的中心点绘制十字标记
                img.draw_cross(red_blob.cx(), red_blob.cy())
                return True
    return False

# 定义红色、紫色的阈值
red_threshold = color_thresholds["Red"]
purple_threshold = color_thresholds["Purple"]

K = 1000  # 测量距离的参数
r = 0 # 判断接收信息的参数
last_time = time.ticks_ms() # 将当前时间赋值给last
interval = 200  # 间隔200 毫秒，即 0.2 秒

# 主循环
while True:
    clock.tick()
    img = sensor.snapshot()  # 拍摄一张照片

    red_balls = find_ball(img, color_thresholds["Red"], "Red", K)
    blue_balls = find_ball(img, color_thresholds["Blue"], "Blue", K)
    yellow_balls = find_ball(img, color_thresholds["Yellow"], "Yellow", K)
    black_balls = find_ball(img, color_thresholds["Black"], "Black", K)

    current_time = time.ticks_ms() # 将当前时间赋值给current
    if time.ticks_diff(current_time, last_time) >= interval:

        # 找不到球 与 找到了球
        if not red_balls and not blue_balls and not yellow_balls and not black_balls:
            uart.write("$")
            print("找不到球")
        else:
            uart.write("@")
            print("找到了球")

        # 错误（蓝）球处理
        for color, balls in [("Blue", blue_balls)]:
            for x, length in balls:
                if length <= 30:
                    length = round(length)
                    uart.write("^")
                    print("捡到对方的球")
                    print(f"检测到{color}小球！横坐标:{x}，距离:{length}")

        # 输出小球信息
        for color, balls in [("Red", red_balls), ("Yellow", yellow_balls), ("Black", black_balls)]:
            for x, length in balls:
                send_coordinates(uart, x, length, color)

        # 先处理黄球
        if yellow_balls:
            if len(yellow_balls) != 1:
                for x, length in yellow_balls:
                    uart.write("^")
                    print("黄球多了")
                    print(f"多个黄球！横坐标:{x}，距离:{length}")
            else:
                for x, length in yellow_balls:
                    # 判断是否捕获黄球
                    if length <= 5:
                        uart.writr("!")
                        print("捕获黄球成功")
                        break
                    # 如果未捕获，则一直循环
                    else:
                        send_coordinates(uart, x, length, color)

                        img = sensor.snapshot()  # 重新拍摄一张照片
                        yellow_balls = find_ball(img, color_thresholds["Yellow"], "Yellow", K)
                    continue
        # 没有黄球
        else:
            all_other_balls = red_balls + black_balls
            if len(all_other_balls) > 4:
                print("球太多了")
                for x, length in all_other_balls:
                    uart.write("^")
                    print("小球多了")
                    print(f"多个小球！横坐标:{x}，距离:{length}")
            elif 1 <= len(all_other_balls) <= 4:
                for x, length in all_other_balls:
                    send_coordinates(uart, x, length, color)

        print(clock.fps())
        last_time = current_time